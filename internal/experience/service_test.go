...
// testServiceSetup 创建使用 mock Repository 的 Service 实例
...
func TestService_Save(t *testing.T) {
	...
	t.Run("合法经验保存成功", func(t *testing.T) {
		result, err := svc.Save(ctx, &Experience{
			Title:   "Go 错误处理",
...
func TestService_Get(t *testing.T) {
	...
	t.Run("获取已存在的经验", func(t *testing.T) {
		saveResult, _ := svc.Save(ctx, &Experience{
			Title:   "获取测试",
			Content: "用于测试 Get 方法",
		}, true)
...
func TestService_Delete(t *testing.T) {
	...
	t.Run("删除后 Get 失败", func(t *testing.T) {
		saveResult, _ := svc.Save(ctx, &Experience{Content: "即将被删除"}, true)
...
func TestService_List(t *testing.T) {
	...
	svc.Save(ctx, &Experience{Content: "经验 1", Source: "claude"}, true)
	svc.Save(ctx, &Experience{Content: "经验 2", Source: "chatgpt"}, true)
