...
func TestRepository_SaveAndGet(t *testing.T) {
	...
	t.Run("Save 返回的 ID 是幂等唯一的", func(t *testing.T) {
		exp1 := Experience{Content: "第一条经验"}
		exp2 := Experience{Content: "第二条经验"}
...
func TestRepository_Delete(t *testing.T) {
	...
	t.Run("删除后 Get 返回错误", func(t *testing.T) {
		exp := Experience{Content: "即将被删除的经验"}
		id, _ := repo.Save(ctx, &exp)
	...
	t.Run("删除后向量也被清除", func(t *testing.T) {
		exp := Experience{Content: "删除后向量也应清除"}
		id, _ := repo.Save(ctx, &exp)
...
func TestRepository_List(t *testing.T) {
	...
	repo.Save(ctx, &Experience{Content: "经验 A", Source: "claude"})
	repo.Save(ctx, &Experience{Content: "经验 B", Source: "chatgpt"})
	repo.Save(ctx, &Experience{Content: "经验 C", Source: "claude"})
...
func TestRepository_Update(t *testing.T) {
	...
	t.Run("更新后能取回新内容", func(t *testing.T) {
		exp := Experience{Title: "旧标题", Content: "旧内容"}
		id, _ := repo.Save(ctx, &exp)
	...
	t.Run("更新不存在的 ID 返回错误", func(t *testing.T) {
		exp := Experience{ID: "nonexistent", Content: "内容"}
		err := repo.Update(ctx, &exp)
		if err == nil {
			t.Error("期望返回错误")
		}
