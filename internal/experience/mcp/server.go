...
// Server MCP Server，暴露经验库能力给 AI Agent
type Server struct {
...
func NewServer(svc *experience.Service) *Server {
	s := &Server{svc: svc}
	s.registerTools()
	return s
}
...
// CallTool 调用指定工具
func (s *Server) CallTool(name string, params map[string]interface{}) (CallToolResult, error) {
